...
// IMPORTANT : le SW est un vrai fichier statique (public/sw.js), PAS une data:
// URL générée au runtime. Chrome/Android refuse d'enregistrer un Service
// Worker depuis une data: URL (origine "opaque", non autorisée pour un SW) —
// l'enregistrement échouait silencieusement (le .catch(()=>{}) masquait
// l'erreur), donc l'app ne remplissait jamais les critères d'installabilité
// PWA. Conséquence : "Ajouter à l'écran d'accueil" créait un simple favori qui
// rouvrait Chrome au lieu de lancer LearnUpr en mode standalone. Enregistrer
// un fichier same-origin résout ça.
(function() {
  ...
  try {
    ...
    }).catch(e => console.warn('Service Worker — échec d\'enregistrement:', e.message));
  } catch(e) {}
})();
